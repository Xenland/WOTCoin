				...
				*/
				...
				function wot_tx_queue_information($tx_id){
					...
					*/
					...
					$transaction_info_q = wot_doQuery("SELECT `id`, `timestamp_initiated`, `address_a_id`, `address_a`, `address_a_signature`, `address_a_signature_fulfilled`, `address_a_status`, `address_b_id`, `address_b`, `address_b_status`, `address_b_signature`, `address_b_signature_fulfilled`, `message_to_sign` FROM `feedback_queue_index` WHERE `id` = ? AND (`address_a` = ? OR `address_b` = ?) LIMIT 0,1", $tx_id, $wot_session["address"], $wot_session["address"]);
